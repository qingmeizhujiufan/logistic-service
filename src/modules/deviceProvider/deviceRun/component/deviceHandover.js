...
// 为日历对象添加属性方法
  showCalendar =(year, month) => {
    ...
    var firstday = this.get_first_date(year, month).getDay();//获取一个月的第一天对应的是星期几
    var lastday = this.get_last_date(year, month).getDay();//获取一个月的最后一天对应的是星期几
    var lastdate = this.get_last_date(year, month).getDate();//获取一个月的最后一天的日期
                  ...
                  return  <ul key = {index1} className = "handover-list handover-hiden" data-day = {dataItem.day}>
                      {
                       ...
                       return (item.id == null) ?  null :(
                          <li key = {index2} className="mylist">
                            <Link to ={{pathname: '/deviceProvider/deviceHandoverRecord/' + item.id + '/' + dataItem.day}}>
                                <div className="login">
